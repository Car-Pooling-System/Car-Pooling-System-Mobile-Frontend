            ...
            >
                ...
                <View style={tw`flex-1 mr-2`}>
                    <View style={tw`flex-row items-center gap-1.5`}>
                        <Text
                            style={[tw`text-sm flex-1`, { color: colors.textPrimary, fontWeight: unread > 0 ? "800" : "600" }]}
                            numberOfLines={1}
                        >
                            {title}
                        </Text>
                        {lastMsg?.sentAt && (
                            <Text style={[tw`text-[10px]`, { color: unread > 0 ? colors.primary : colors.textMuted }]}>
                    ...
                    {lastMsg?.text ? (
                        <Text
                            style={[tw`text-xs mt-0.5`, { color: unread > 0 ? colors.textPrimary : colors.textSecondary, fontWeight: unread > 0 ? "600" : "400" }]}
                            numberOfLines={1}
                        >
                            {isMine ? "You: " : isGroup ? `${lastMsg.senderName?.split(" ")[0] || ""}: ` : ""}
                            {lastMsg.text}
                        </Text>
                    ) : (
                        <Text style={[tw`text-xs mt-0.5 italic`, { color: colors.textMuted }]}>
                            No messages yet
                        </Text>
                    )}
                </View>
    ...
    return (
        <View style={[tw`flex-1`, { backgroundColor: colors.background }]}>
            ...
            <View style={[tw`px-5 pt-4 pb-3 flex-row items-center gap-3 border-b`, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <TouchableOpacity onPress={() => router.back()} style={tw`mr-1`}>
            ...
            {conversations.length === 0 ? (
                <View style={tw`flex-1 items-center justify-center px-8`}>
                    ...
                    <Text style={[tw`text-sm text-center mt-1`, { color: colors.textSecondary }]}>
                        Start a chat from a ride&apos;s details page — tap the chat button next to any passenger or driver.
                    </Text>
                </View>
            ) : (
                <FlatList
