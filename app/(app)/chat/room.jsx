    ...
    const fetchMessages = useCallback(async (pg = 1) => {
        ...
        try {
            const res = await fetch(
                `${BACKEND_URL}/api/chat/messages/${conversationId}?page=${pg}&limit=50`,
            );
            const data = await res.json();
            if (res.ok) {
                if (pg === 1) {
    ...
    useEffect(() => {
        ...
        const onTyping = ({ userId: uid, senderName, conversationId: cid }) => {
            if (cid === conversationId && uid !== user?.id) {
                setTypingUsers((prev) =>
                    prev.some((t) => t.userId === uid)
                        ? prev
                        : [...prev, { userId: uid, name: senderName }],
                );
            }
        };
            ...
            <Pressable onPress={Keyboard.dismiss}>
                {showDate && (
                    <View style={tw`items-center my-3`}>
                        <View style={[tw`px-3 py-1 rounded-full`, { backgroundColor: colors.surfaceMuted }]}>
                            <Text style={[tw`text-[10px] font-bold`, { color: colors.textMuted }]}>
                ...
                <View style={[tw`flex-row px-4`, isMine ? tw`justify-end` : tw`justify-start`, showTime ? tw`mb-1.5` : tw`mb-0.5`]}>
                    ...
                    <View style={[tw`max-w-[75%]`]}>
                        {showName && (
                            <TouchableOpacity onPress={handleAvatarPress} activeOpacity={0.7}>
                                <Text style={[tw`text-[10px] font-bold mb-0.5 ml-1`, { color: colors.textSecondary }]}>
                                    {msg.senderName?.split(" ")[0] || "User"}
                                </Text>
                            </TouchableOpacity>
                        )}
                        <View
                            style={[
                                ...
                                isMine
                                    ? [{ backgroundColor: colors.primary }, tw`rounded-br-md`]
                                    : [{ backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border }, tw`rounded-bl-md`],
                            ]}
                        >
                            <Text style={[tw`text-sm leading-5`, { color: isMine ? "#fff" : colors.textPrimary }]}>
                                {msg.text}
                            </Text>
                        </View>
                        {showTime && (
                            <Text
                                style={[
                                    tw`text-[9px] mt-0.5 mx-1`,
                                    { color: colors.textMuted, textAlign: isMine ? "right" : "left" },
                                ]}
                            >
        ...
        <View style={[tw`flex-1`, { backgroundColor: colors.background }]}>
            ...
            <View style={[tw`flex-row items-center px-4 pt-3 pb-3 border-b`, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                ...
                >
                    ...
                    <View style={tw`flex-1`}>
                        <Text style={[tw`text-base font-bold`, { color: colors.textPrimary }]} numberOfLines={1}>
                            {title || "Chat"}
                        </Text>
            ...
            <FlatList
                ...
                ListHeaderComponent={
                    loadingMore ? (
                        ...
                        <TouchableOpacity onPress={loadMore} style={tw`py-3 items-center`}>
                            <Text style={[tw`text-xs font-bold`, { color: colors.primary }]}>Load older messages</Text>
                        </TouchableOpacity>
                    ) : null
                }
                ListEmptyComponent={
            ...
            <View style={[tw`flex-row items-end px-4 py-2 border-t`, {
                ...
                <TextInput
                    ...
                    maxLength={2000}
                    style={[
                        tw`flex-1 text-sm px-4 py-2.5 rounded-2xl mr-2`,
                        {
